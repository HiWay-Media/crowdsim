...
```bash
docker pull ghcr.io/hiway-media/crowdsim:1.14.1     # exact version — use this
...
docker run --rm --network host \
  ...
  -v "$PWD/out:/out" \
  ghcr.io/hiway-media/crowdsim:1.14.1 \
...
docker run --rm --network host -e CROWDSIM_ALLOW_TARGETS='www.example.test' \
  ...
  ghcr.io/hiway-media/crowdsim:1.14.1 crowdsim probe --profile /profile.json
