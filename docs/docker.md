...
```bash
docker pull ghcr.io/hiway-media/crowdsim:1.38.0     # exact version — use this
docker pull ghcr.io/hiway-media/crowdsim:1.38       # latest patch of 1.2
...
docker run --rm --network host \
  ...
  -v "$PWD/out:/out" \
  ghcr.io/hiway-media/crowdsim:1.38.0 \
...
docker run --rm --network host -e CROWDSIM_ALLOW_TARGETS='www.example.test' \
  ...
  ghcr.io/hiway-media/crowdsim:1.38.0 crowdsim probe --profile /profile.json
...
docker run --rm -v "$PWD/my-profile.json:/profile.json:ro" -v /var/log/nginx:/logs:ro \
  ghcr.io/hiway-media/crowdsim:1.38.0 crowdsim weights /logs/access.log --profile /profile.json
...
ssh edge 'zcat /var/log/nginx/access.log.*.gz' \
  | docker run --rm -i -v "$PWD/my-profile.json:/profile.json:ro" \
      ghcr.io/hiway-media/crowdsim:1.38.0 crowdsim weights - --profile /profile.json
```
