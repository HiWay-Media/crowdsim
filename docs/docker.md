...
```bash
docker pull ghcr.io/hiway-media/crowdsim:1.33.1     # exact version — use this
...
docker run --rm -p 127.0.0.1:8787:8787 \
  ...
  -v "$PWD/out:/out" \
  ghcr.io/hiway-media/crowdsim:1.33.1 crowdsim serve
```
...
docker run --rm --network host \
  ...
  -v "$PWD/out:/out" \
  ghcr.io/hiway-media/crowdsim:1.33.1 \
...
docker run --rm --network host -e CROWDSIM_ALLOW_TARGETS='www.example.test' \
  ...
  ghcr.io/hiway-media/crowdsim:1.33.1 crowdsim probe --profile /profile.json
...
docker run --rm -v "$PWD/my-profile.json:/profile.json:ro" -v /var/log/nginx:/logs:ro \
  ghcr.io/hiway-media/crowdsim:1.33.1 crowdsim weights /logs/access.log --profile /profile.json
...
ssh edge 'zcat /var/log/nginx/access.log.*.gz' \
  | docker run --rm -i -v "$PWD/my-profile.json:/profile.json:ro" \
      ghcr.io/hiway-media/crowdsim:1.33.1 crowdsim weights - --profile /profile.json
```
