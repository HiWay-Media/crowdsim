 ...
 */
export function authedTargets(profile, opts) {
  const p = profile || {};
  // A SAMPLE per class, spread across the pool rather than the first n: the first entries of a pool are
  // the ones most likely to exist, and one endpoint answering 401 does not establish that the other 399
  // require the token. Default 1, which is what this did before sampling existed. (#77)
  const want = Math.max(1, Number((opts || {}).sample) || 1);
  const pools = p.pools || {};
  const targets = [];
  const skipped = [];
  for (const c of p.classes || []) {
    ...
    if (n === 1) {
      ...
      for (var k = 0; k < n; k++) {
        const at = Math.round(k * (pool.length - 1) / (n - 1));
        if (picked.indexOf(at) === -1) picked.push(at);
      }
    }
    for (var q = 0; q < picked.length; q++) {
      targets.push({ class: name, pool: c.pool, path: String(pool[picked[q]]) });
    }
  }
