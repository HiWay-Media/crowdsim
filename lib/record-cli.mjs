...
if (journey.think_time) {
  const tt = journey.think_time.samples;
  const lo = Math.min.apply(null, tt);
  const hi = Math.max.apply(null, tt);
  console.log(`     ${tt.length} reading pauses measured (${lo}-${hi} ms) — the generator will replay these`);
  console.log('             instead of its default 1-5s. Session duration is half of any concurrency');
  console.log('             figure, so a measured pause is the difference between a number and a guess');
}
console.log('');
