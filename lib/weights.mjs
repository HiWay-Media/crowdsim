...
/**
 * Kinds this command cannot count, and why it must say so rather than report zero.
 *
 * `login` and `signup` are POSTs. This command counts GETs only — deliberately, because this tool sends
 * GETs and a write in the mix would be a weight for load that will never be generated — so no access log
 * can ever produce a number for them.
 *
 * Until 1.20.5 they were mapped to `plain` along with everything that was not `rsc`, which was true when
 * those were the only two kinds. The result was not that they matched nothing: a `login` class declared
 * before the document class, on the same pool, matched EVERY document GET in the log and took them from
 * the class that had served them. A log with three page views and one login produced a mix of 100% login.
 *
 * `authed` is not here: an authenticated read is a GET, and whether a log can tell it apart from an
 * anonymous one is the operator's job to declare with `log_match`.
 */
export const UNCOUNTABLE_KINDS = ['login', 'signup'];
  ...
  return {
    ...
    classes: classes.map((c) => {
      ...
      return {
        ...
        // A write kind is carried through so it can be REPORTED, and never matched. See UNCOUNTABLE_KINDS.
        countable: UNCOUNTABLE_KINDS.indexOf(c.kind) === -1,
        declaredKind: c.kind || 'plain',
...
export function classifyRequest(rec, rules) {
  ...
  for (const c of rules.classes) {
    // A class this command cannot count never matches. Letting it fall through to the `plain` rules is
    // what made a login class swallow every document GET in the log.
...
export function result(tally, opts) {
  ...
  const classes = tally.rules.classes.map((c) => {
    ...
    return {
      ...
      declared: c.declared,
      // Zero and "cannot be counted" are different findings with different fixes: one means this window
      // did not contain the class, the other means no GET log ever will.
      countable: c.countable,
      declared_kind: c.declaredKind,
    };
  });
  ...
  return {
    ...
    uncountable: tally.rules.classes.filter((c) => !c.countable).map((c) => c.name),
  };
}
