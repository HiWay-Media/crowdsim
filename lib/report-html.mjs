...
export const W = 760;
export const H = 300;
export const PAD = { top: 24, right: 96, bottom: 52, left: 62 };
...
// ── 3b. which class answered what ────────────────────────────────────────────────────────────────────

/**
 * Outcomes per class, as a stacked bar of counts.
 *
 * WHY — the failure-mode line's most useful claim is *concentrated on 2 of 5 classes*: the same code on
 * some classes and not others points at a pool or a route, spread evenly it points at the system. That
 * distinction was prose everywhere and a chart nowhere, next to a chart of p95 — so a run serving 474
 * 404s on two classes drew a perfectly healthy set of bars. It is the one thing a chart is actually for.
 *
 ...
 *    was fine; a class that never ran is a different statement.
 ...
 *    nobody can read.
 *  · **counts, never shares recomputed here.** `failure_mode.share` and these bands come from the same
 *    numbers, or they will disagree on the same page.
 *
 * Every band carries its own CSS class and its own <title>, so it survives being printed in greyscale —
 * which is what happens to a page whose whole point is that it travels.
 */
export function outcomeChart(perClass) {
  const all = Object.keys(perClass || {});
  // Absent, not zero: see above.
  const names = all.filter((c) => perClass[c] && num0(perClass[c].reqs) > 0);
  if (!names.length) return '';

  // The arithmetic is in k6/lib/failure.js, next to the code list, because the GUI draws this too and two
  // copies of "cs_5xx is the superset" is the duplication this project keeps paying for.
  const bandsFor = (name) => outcomeBands(perClass[name]);

  const anyFailure = names.some((n) => bandsFor(n).some((b) => b.key !== 'ok' && b.n > 0));
  // A run where nothing failed gets no chart: six identical full-width bars say nothing the p95 chart
  // does not already say better.
  if (!anyFailure) return '';

  const rowH = 30;
  const height = PAD.top + names.length * rowH + 30;
  const x0 = PAD.left + 40;
  const rows = names.map((name, i) => {
    const y = PAD.top + i * rowH;
    const bands = bandsFor(name);
    const total = bands.reduce((n, b) => n + b.n, 0) || 1;
    const x = linear([0, total], [x0, W - PAD.right]);
    let at = 0;
    const segs = bands.filter((b) => b.n > 0).map((b) => {
      const from = x(at);
      at += b.n;
      const w = Math.max(1, x(at) - from);
      return `<rect class="band ${b.key}" x="${from.toFixed(1)}" y="${y}" width="${w.toFixed(1)}" `
        + `height="14"><title>${esc(name)}: ${b.n} ${esc(b.label)} of ${total}</title></rect>`;
    }).join('');
    const failed = bands.filter((b) => b.key !== 'ok').reduce((n, b) => n + b.n, 0);
    return `<text class="tick" x="${PAD.left + 34}" y="${y + 11}" text-anchor="end">${esc(name)}</text>`
      + segs
      + `<text class="bar-value" x="${(W - PAD.right + 4).toFixed(1)}" y="${y + 11}">`
      + `${failed ? esc(String(failed)) : ''}</text>`;
  }).join('');

  const desc = names.map((n) => `${n}: ` + bandsFor(n).filter((b) => b.n > 0)
    .map((b) => `${b.n} ${b.label}`).join(', ')).join('; ');
  return `<figure class="chart">\n<svg viewBox="0 0 ${W} ${height}" role="img" aria-labelledby="out-t out-d">`
    + '<title id="out-t">what each request class answered</title>'
    + `<desc id="out-d">${esc(desc)}</desc>`
    + rows
    + `<text class="axis-label" x="${x0}" y="${height - 6}">requests per class, by outcome — the number `
    + 'on the right is that class\'s failures</text>'
    + '</svg>\n</figure>';
}

// ── 3c. a server-side series, against the same steps ─────────────────────────────────────────────────

/**
 * A handed-in server-side series, drawn on the ramp's own step axis with latency beside it. (#86)
 *
 * WHY — `--server-metrics` (1.31.0) aligns the series to the run's steps and reported it as a table of
 * mean and max. Reading it *against* latency is the entire reason for aligning it, and doing that from
 * two tables means comparing rows by eye. Both halves already share an axis.
 *
 * What this must not do is imply causation by construction:
 *
 *  · **two scales, both named.** The series has its own units and its own axis on the right. One shared
 *    axis would make two different quantities look like one.
 ...
 *    paragraph next to it, and the word is *correlation*.
 *  · **a step with no samples is a GAP.** The line breaks rather than spanning a window nobody recorded,
 *    which would draw data that does not exist.
 *
 * `opts.perStep` supplies the latency; omit it (or pass none) on an invalid run and only the series is
 * drawn, for the same reason the failure mode is still shown there: a counter that rose is not a latency
 * claim.
 */
export function seriesChart(serverSide, opts) {
  const o = opts || {};
  const ss = serverSide || {};
  const rows = Array.isArray(ss.steps) ? ss.steps.filter((r) => r && num0(r.samples) > 0) : [];
  const perStep = Array.isArray(o.perStep) ? o.perStep : null;
  if (!rows.length || !perStep || !perStep.length) return '';

  // The x axis is the ramp's steps, in the order the run ran them: that is the axis both halves share.
  const order = perStep.map((r) => r.step);
  const at = (step) => order.indexOf(step);
  const n = Math.max(1, order.length - 1);
  const x = linear([0, n], [PAD.left + 46, W - PAD.right - 46]);

  const sMax = niceCeil(Math.max(...rows.map((r) => num0(r.max))) * 1.1) || 1;
  const ys = linear([0, sMax], [H - PAD.bottom, PAD.top]);

  // Latency, when this run is one whose latency means anything.
  const lat = perStep.filter((r) => r && r.p95 !== null && r.p95 !== undefined);
  const pMax = lat.length ? (niceCeil(Math.max(...lat.map((r) => r.p95)) * 1.1) || 1) : 0;
  const yp = linear([0, pMax || 1], [H - PAD.bottom, PAD.top]);

  // Segments, so a missing step breaks the line instead of being spanned.
  const segments = [];
  let current = [];
  for (const step of order) {
    ...
    current.push(`${x(at(step)).toFixed(1)},${ys(num0(row.mean)).toFixed(1)}`);
  }
  if (current.length) segments.push(current);
  const seriesLines = segments.filter((seg) => seg.length)
    .map((seg) => `<polyline class="series-line" fill="none" points="${seg.join(' ')}"/>`).join('');
  const dots = rows.map((r) => `<circle class="series-pt" cx="${x(at(r.step)).toFixed(1)}" `
    + `cy="${ys(num0(r.mean)).toFixed(1)}" r="3"><title>${esc(r.step)}: mean ${num0(r.mean)}, max `
    + `${num0(r.max)} over ${num0(r.samples)} samples${r.partial ? ' (partial step)' : ''}`
    + '</title></circle>').join('');

  const p95Line = lat.length > 1
    ? `<polyline class="series-p95" fill="none" points="${lat
      .map((r) => `${x(at(r.step)).toFixed(1)},${yp(r.p95).toFixed(1)}`).join(' ')}"/>`
    : '';

  const xLabels = order.map((step, i) => `<text class="tick" x="${x(i).toFixed(1)}" `
    + `y="${H - PAD.bottom + 14}" text-anchor="middle">${esc(step)}</text>`).join('');

  const desc = rows.map((r) => `${r.step}: mean ${num0(r.mean)}, max ${num0(r.max)}`).join('; ');
  return `<figure class="chart">\n<svg viewBox="0 0 ${W} ${H}" role="img" aria-labelledby="ss-t ss-d">`
    + `<title id="ss-t">${esc(ss.label)} per step${lat.length ? ', with p95' : ''}</title>`
    + `<desc id="ss-d">${esc(desc)}</desc>`
    + `<line class="axis" x1="${PAD.left + 46}" y1="${H - PAD.bottom}" x2="${W - PAD.right - 46}" `
    + `y2="${H - PAD.bottom}"/>`
    + xLabels + p95Line + seriesLines + dots
    + `<text class="axis-label" x="${PAD.left + 46}" y="${PAD.top - 10}">${esc(ss.label)} (left, `
    + `0–${sMax})</text>`
    + (lat.length ? `<text class="axis-label" x="${W - PAD.right - 46}" y="${PAD.top - 10}" `
      + `text-anchor="end">p95 ms (right, 0–${pMax})</text>` : '')
    + `<text class="axis-label" x="${PAD.left + 46}" y="${H - 6}">a correlation: the series moved during `
    + 'these windows, which is a reason to look and not a finding on its own</text>'
    + '</svg>\n</figure>';
}
...
.delta.better { fill:var(--ok); }
.delta.worse { fill:var(--bad); }
.delta.same { fill:var(--muted); }
.series-line { stroke:var(--accent); stroke-width:2; }
.series-pt { fill:var(--accent); }
/* Dashed, so the two quantities are not read as one series even in greyscale. */
.series-p95 { stroke:var(--muted); stroke-width:1.5; stroke-dasharray:4 3; }
.band.ok { fill:var(--ok); }
.band.e404 { fill:var(--warn); }
.band.e504 { fill:var(--bad); }
.band.e502 { fill:var(--bad); opacity:.72; }
.band.e5xx { fill:var(--bad); opacity:.5; }
.band.denied { fill:var(--accent); }
/* Printed in greyscale the fills collapse, so every failure band also carries a hatch. */
.band.e404, .band.e502, .band.denied { stroke:var(--panel); stroke-width:.6; stroke-dasharray:2 2; }
 ...
 */
/**
 * WHAT THIS PAGE DRAWS, and what it deliberately does not.
 *
 * The GUI accumulated six unrendered summary blocks because nothing noticed, and got two exhaustive lists
 * and a test for it in 1.34.0. This page was left without one — and by then it was already missing
 * `drop_diagnosis`, so the artefact most likely to be attached to a ticket still opened with DISCARD THIS
 * RUN for a target that had simply saturated: the advice 1.32.0 exists to correct.
 *
 * `tests/unit/report-html.test.js` asks `buildSummary` itself for its keys and fails on a block that
 * belongs to neither list. A block left out on purpose is a decision; one left out because nobody looked
 * is the bug. (#84)
 */
/** A counter, where 0 is a real answer — unlike a latency, where absent and zero differ. */
const num0 = (v) => (v === null || v === undefined || !isFinite(Number(v)) ? 0 : Number(v));

export const DRAWN = [
  'run_id', 'profile', 'base_url', 'shape', 'peak_rps_user_target',
  'generator_ok', 'target_unreachable', 'dropped_iterations', 'requests',
  'drop_diagnosis', 'failure_mode', 'delivery', 'knee',
  'aborted', 'aborted_by', 'failed_rate', 'dur', 'rps_avg',
  'guillotine_ms', 'over_guillotine_rate', 'slo',
  'e504', 'e502', 'e5xx', 'e404', 'denied', 'authFail',
  'cache', 'per_class', 'per_step', 'warmup', 'is_warmup',
];

export const DELIBERATELY_NOT_DRAWN = [
  // Journey-shape only. Both are numbers rather than shapes, and `report` (markdown) carries them with
  // the caveat that makes them citable — drawing two concurrency methods on one axis would be the
  // averaging this tool refuses.
  'concurrency',
  'think_time',
  // The per-class rate table, which `mix_target` already draws. Two pictures of one arithmetic is the
  // disagreement these lists exist to prevent.
  'allocation',
  'mix_target',
  // A registration manifest names real accounts on a real system: it belongs in out/, not in a page
  // written to be attached to a ticket.
  'signup',
  'auth',
  // Its own artefact and its own chart, once #86 lands. Listed so that adding the chart and forgetting
  // this list cannot pass silently.
  'server_side',
  // A profile setting, not a measurement: it belongs to the run's identity line, which `report`
  // (markdown) prints. Drawing it would be drawing an input.
  'rsc_mode',
];
...
export function buildReport(summary, opts) {
  ...
  if (invalid) {
    // WHICH kind of invalid. `generator_ok: false` covers two opposite causes, and telling somebody to
    // discard a run whose target simply saturated throws away the finding they booked the window for.
    // The verdict decides the words; the no-latency-charts rule is unchanged either way, because a rate
    // that was not delivered was not measured. (#84)
    const dd = s.drop_diagnosis;
    if (dd && dd.verdict === 'target') {
      parts.push('<div class="banner warn"><strong>The TARGET could not absorb the requested rate.</strong> '
        + `${esc(dd.reason)} ${esc(dd.fix)} `
        + `The generator dropped ${esc(s.dropped_iterations)} iterations of ${esc(s.requests)} requests, `
        + 'so no step measured the rate it claims and there are no latency charts here — but this is a '
        + 'finding about the target, not a wasted window.</div>');
    } else if (dd && dd.verdict === 'generator') {
      parts.push('<div class="banner bad"><strong>DISCARD THIS RUN.</strong> '
        + `${esc(dd.reason)} ${esc(dd.fix)} `
        + `The generator dropped ${esc(s.dropped_iterations)} iterations of ${esc(s.requests)} requests. `
        + 'A generator-bound run looks exactly like a healthy system absorbing load, which is why there '
        + 'are no latency charts on this page.</div>');
    } else {
      parts.push('<div class="banner bad"><strong>DISCARD THIS RUN.</strong> The generator dropped '
        ...
        + 'there are no latency charts on this page. '
        + (dd ? `${esc(dd.reason)} ${esc(dd.fix)}` : 'Move the generator closer to the target, or onto a '
          + 'bigger host, and repeat.') + '</div>');
    }
  } else if (unreachable) {
  ...
  // ── 1b. WHAT BROKE, before what stopped the run ──────────────────────────────────────────────────
  // A page that opened on "stopped by class html p95" sent its reader looking for a slow renderer while
  // the finding was 6.31% 404s on the frontend classes alone. Both true; the order is what misleads.
  // Shown even on an invalid run: a failure mode is not a latency claim, so the no-charts rule does not
  // apply to it — what broke is worth knowing about a run whose numbers are discarded. (#74)
  if (s.failure_mode) {
    parts.push('<h2>Failure mode</h2>');
    parts.push(`<div class="banner warn">${esc(s.failure_mode.line)}</div>`);
  }

  // ── 1c. requested and delivered ──────────────────────────────────────────────────────────────────
  // `--peak` is total USER requests per second, and one user request fans out into several HTTP requests.
  // A page that shows only the requested rate leaves the number the target actually survived to the
  // reader's arithmetic. (#71)
  if (s.delivery) {
    parts.push('<h2>Requested and delivered</h2>');
    if (s.delivery.refused) {
      parts.push(`<p class="note">No delivered rate from this run: ${esc(s.delivery.reason)}</p>`);
    } else {
      parts.push(`<p><strong>${esc(s.delivery.requested_rps)} req/s requested → `
        + `${esc(s.delivery.delivered_rps)} req/s arrived at the target.</strong></p>`);
      if (s.delivery.note) parts.push(`<p class="note">${esc(s.delivery.note)}</p>`);
    }
  }

  // ── 1d. a server-side series, if one was handed to this run ──────────────────────────────────────
  // Drawn on the ramp's own step axis, with latency beside it on its own scale — and on an invalid run
  // the series is drawn without the latency, for the same reason the failure mode is: a counter that
  // rose is not a latency claim. (#86)
  if (o.serverSide && Array.isArray(o.serverSide.steps) && o.serverSide.steps.length) {
    parts.push(`<h2>${esc(o.serverSide.label)}, against the ramp</h2>`);
    parts.push('<p class="note">Handed to this run, not collected: crowdsim does not fetch server-side '
      + 'metrics. What this shows is a <strong>correlation</strong> — the series moved during these '
      + 'windows, which is a reason to look and not a finding on its own.'
      + (o.serverSide.source ? ` Source: <code>${esc(o.serverSide.source)}</code>.` : '') + '</p>');
    parts.push(seriesChart(o.serverSide, { perStep: noPerf ? null : perStep }));
    if (noPerf) {
      parts.push('<p class="note">No latency beside it: this run did not deliver the rate it asked for, '
        + 'so its latency describes the generator or the target rather than the system at that rate.</p>');
    }
  }
  ...
  // ── the unit the requirement was written in ─────────────────────────────────────────────────────
  // Two numbers or a refusal, never one figure: the agreement between the methods is what makes either
  // worth quoting. See k6/lib/session.js.
  if (s.concurrency) {
    const c = s.concurrency;
    parts.push('<h2>Concurrent users</h2>');
    if (c.refused) {
      parts.push(`<div class="banner warn"><strong>No concurrency figure from this run.</strong> `
        + `${esc(c.reason)} ${esc(c.fix || '')}</div>`);
    } else {
      parts.push('<div class="stats">'
        + stat('derived', num(c.derived), 'rate × mean session (Little\'s law)')
        + stat('in flight', num(c.observed), 'peak sessions at once, counted')
        + stat('sessions/s', c.sessions_per_sec === null ? 'n/a' : c.sessions_per_sec.toFixed(1))
        + stat('mean session', c.mean_session_seconds === null ? 'n/a'
          : `${c.mean_session_seconds.toFixed(1)} s`)
        + '</div>');
      if (c.vu_bound || c.agree === false) {
        parts.push(`<div class="banner bad"><strong>These two do not agree.</strong> ${esc(c.note)}</div>`);
      } else if (c.agree === true) {
        parts.push('<div class="banner ok"><strong>The two methods agree.</strong> That agreement is what '
          + 'makes the number worth quoting — one method alone cannot tell a measurement from an '
          + 'artefact of the arithmetic.</div>');
      }
      if (c.caveat) parts.push(`<p class="note">${esc(c.caveat)}</p>`);
    }
  }
  ...
  parts.push('<div class="stats">'
    ...
    + (s.signup && s.signup.created ? stat('accounts created', num(s.signup.created), 'they still exist') : '')
  ...
  // ── 3b. WITH WHAT each class failed ──────────────────────────────────────────────────────────────
  // p95 per class was the only thing a class did on this page, so a run serving 474 × 404 on two classes
  // drew a perfectly healthy set of bars. The counters have been in the summary since 1.29.0 and reached
  // nothing that gets attached to a ticket. Shown on an INVALID run too: a 404 does not become untrue
  // because the rate was not delivered. (#84)
  if (s.per_class && Object.keys(s.per_class).length) {
    const withErrors = Object.keys(s.per_class).filter((c) => {
      const e = (s.per_class[c] || {}).errors || {};
      return num0(e.e504) + num0(e.e502) + num0(e.e5xx) + num0(e.e404) + num0(e.denied) > 0;
    });
    const anyRun = num0(s.e504) + num0(s.e502) + num0(s.e5xx) + num0(s.e404) + num0(s.denied)
      + num0(s.authFail);
    if (withErrors.length || anyRun) {
      parts.push('<h2>What answered, and with what</h2>');
      parts.push('<p class="note">A class answering 404 at volume drags its p95 with it, so the bars '
        + 'above can look like latency when the finding is a route. The same code on some classes and '
        + 'not others points at a pool; spread evenly it points at the system.</p>');
      parts.push(outcomeChart(s.per_class));
      parts.push(table('per class', ['class', 'requests', '404', '5xx', '502', '504', '401/403'],
        Object.keys(s.per_class).map((c) => {
          const e = (s.per_class[c] || {}).errors || {};
          return [c, num(s.per_class[c].reqs), num0(e.e404), num0(e.e5xx), num0(e.e502), num0(e.e504),
            num0(e.denied)];
        })));
      parts.push(`<p class="note">Across the run: ${num0(s.e404)} × 404, ${num0(s.e5xx)} × 5xx `
        + `(${num0(s.e502)} of them 502, ${num0(s.e504)} read timeouts), ${num0(s.denied)} × 401/403`
        + (num0(s.authFail) ? `, and ${num0(s.authFail)} logins that returned no usable token` : '')
        + '.</p>');
    }
  }
  ...
  // A run that created accounts is not finished when the numbers are in.
  if (s.signup && s.signup.created) {
    caveats.push(`<strong>This run created ${esc(s.signup.created)} accounts on the target, and they `
      + 'still exist.</strong> crowdsim will not delete them: removing accounts from an identity provider '
      + 'is not a load generator\'s job, and a tool that could do it would be a tool that could do it by '
      + `accident. Find them with <code>${esc(s.signup.email_glob)}</code>, listed in `
      + `<code>signups-${esc(s.run_id)}.json</code> next to this run's summary.`);
  }
  if (s.think_time) {
    const t = s.think_time;
    caveats.push(`<strong>The reading pauses were ${esc(t.source)}</strong> (${esc(t.min_ms)}-`
      + `${esc(t.max_ms)} ms). Session duration is the fan-out plus these pauses, so any concurrency `
      + 'figure above rests on them: change the pace and the same traffic becomes a different number of '
      + 'users.');
  }
...
/**
 * The page shell, shared so that a second page cannot drift from the first: same stylesheet, same
 * self-contained promise, same footer wording about fetching nothing. Used by buildReport above and by
 * lib/trend-html.mjs. (#87)
 */
export function page(title, bodyParts, footer) {
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
