...
// The authenticated-class rules live with the generator, in k6/lib/auth.js, and are imported
// here rather than restated: a lint that drifts from the runtime is worse than no lint.
import { validateAuth, usesAuth } from '../k6/lib/auth.js';/*
...
export function validateProfile(profile) {
  ...
  for (let i = 0; i < classes.length; i++) {
    ...
    // A class is aimed either by its share of the mix or by an absolute rate — not both. `rate_rps` exists
    // because a finding is usually about one class ("the login saturates at ~150 login/s"), and solving
    // for that rate through a weight is arithmetic in the wrong direction.
    const hasRate = c.rate_rps !== undefined && c.rate_rps !== null;
    if (hasRate) {
      const r = Number(c.rate_rps);
      if (!Number.isFinite(r) || r <= 0) {
        err(`${at}.rate_rps`, 'rate_rps is a positive number of requests per second for this class alone');
      }
      if (c.weight !== undefined && c.weight !== null) {
        err(at, 'a class declares a weight OR a rate_rps, not both: with both, one of them is silently '
          + 'ignored and the run aims somewhere nobody chose');
      }
    } else {
    ...
    const KINDS = ['plain', 'rsc', 'login', 'authed', 'signup'];
    if (c.kind && !KINDS.includes(c.kind)) err(at, `kind must be one of: ${KINDS.join(', ')}`);
    for (const key of ['pool', 'path_suffix_pool']) {
      if (c[key] && !(c[key] in pools)) err(`${at}.${key}`, `unknown pool "${c[key]}"`);
    }
    // `login` and `signup` take their URL from the auth block, not from a pool: requiring one would
    // force profiles to invent a pool that is never read.
    const needsPool = c.kind !== 'login' && c.kind !== 'signup';
    if (!c.pool && needsPool) err(at, 'a class needs a pool to draw URLs from');
    else if (pools[c.pool] === 0) warn(at, `pool "${c.pool}" is empty: this class will be dropped and the mix renormalised`);

    // `log_match` is how `crowdsim weights` recognises this class in an access log. It has no effect on a
    // run: a profile without it generates exactly the same traffic, it just cannot have its mix measured.
    if (c.log_match !== undefined) {
      if (!Array.isArray(c.log_match)) {
        err(`${at}.log_match`, 'log_match is a list of path globs, e.g. ["/_next/static/*"]');
      } else if (!c.log_match.length) {
        warn(`${at}.log_match`, 'an empty log_match matches nothing: remove it, or give it a pattern');
      } else {
        for (let g = 0; g < c.log_match.length; g++) {
          const pat = c.log_match[g];
          if (typeof pat !== 'string' || !pat) {
            err(`${at}.log_match[${g}]`, 'every pattern is a non-empty string');
          } else if (pat[0] !== '/') {
            // Anchored at the start of the path, always. A pattern that does not start with `/` silently
            // matches nothing, and an unclassified share is a bad way to find that out.
            err(`${at}.log_match[${g}]`, `"${pat}" does not start with "/": patterns are matched against the `
              + 'request path from its first character, so this one can never match');
          }
        }
      }
    }
  ...
  const pinnedCount = classes.filter((c) => c && c.rate_rps !== undefined && c.rate_rps !== null).length;
  if (weightTotal <= 0 && classes.length && pinnedCount < classes.length) {
    err('classes', 'the weights add up to zero: nothing would be generated');
  }
  if (pinnedCount && pinnedCount === classes.length) {
    // Legitimate — "these exact rates" is a real thing to ask for — but --peak then stops being the
    // target, and a run that generates 200 next to a --peak of 500 needs to have said so in advance.
    warn('classes', 'every class declares a rate_rps, so --peak becomes a ceiling rather than a target: '
      + 'the run generates the sum of the fixed rates. The safe-peak gate still reads --peak.');
  }

  // ── authenticated classes ─────────────────────────────────────────────────────────────────────────
  // Same rules the generator applies at run time (k6/lib/auth.js). The one difference is the credentials
  // file: at run time it is mandatory, here it is only a warning, because the path normally arrives as
  // CROWDSIM_AUTH_USERS when the run starts and does not belong in a profile that gets shared.
  if (usesAuth(classes)) {
    const pretendCreds = { CROWDSIM_AUTH_USERS: (p.auth && p.auth.users_csv) || '@runtime' };
    for (const msg of validateAuth(p, pretendCreds)) err('auth', msg);
    if (!(p.auth && p.auth.users_csv)) {
      warn('auth', 'no credentials file in the profile: set CROWDSIM_AUTH_USERS=<path> when you run, ' +
                   'or the login class has nothing to sign in with');
    }
  }

  // ── journey.think_time ────────────────────────────────────────────────────────────────────────────
  // Half of any concurrency figure. A typo here does not fail a run: it silently falls back to the tool's
  // default 1-5s, and the number that comes out describes visitors reading at a pace nobody chose.
  const tt = (p.journey || {}).think_time;
  if (tt !== undefined && tt !== null) {
    if (typeof tt !== 'object' || Array.isArray(tt)) {
      err('journey.think_time', 'think_time is an object: { min_ms, max_ms } or { samples: [ms, …] }');
    } else if (tt.samples !== undefined) {
      if (!Array.isArray(tt.samples) || !tt.samples.length) {
        err('journey.think_time.samples', 'samples is a non-empty list of pauses in milliseconds');
      } else if (!tt.samples.every((v) => Number.isFinite(Number(v)) && Number(v) > 0)) {
        err('journey.think_time.samples', 'every pause is a positive number of milliseconds — a 0 or a '
          + 'null would silently remove the pause it was meant to describe');
      } else if (!tt.measured) {
        warn('journey.think_time', 'samples without `measured: true` are reported as declared rather than '
          + 'measured: `crowdsim record` sets both when it takes them from a recording');
      }
    } else {
      const lo = Number(tt.min_ms);
      const hi = Number(tt.max_ms);
      if (!Number.isFinite(lo) || !Number.isFinite(hi) || lo < 0 || hi < lo) {
        err('journey.think_time', 'min_ms and max_ms are milliseconds with max_ms >= min_ms >= 0 — an '
          + 'invalid range falls back to the default 1000-5000 ms without saying so');
      }
    }
  }

  // ── rsc ───────────────────────────────────────────────────────────────────────────────────────────
  // The generator reads `rsc.param` (k6/live-event.js) and nothing reads `rsc.query`. A profile that spells
  // it the second way silently uses the default `_rsc`, which for a site that named its parameter something
  // else means every navigation request in the run is a URL that site never serves.
  const rscCfg = p.rsc || {};
  if (rscCfg.query !== undefined && rscCfg.param === undefined) {
    const lost = String(rscCfg.query) !== '_rsc'
      ? ` This run would send "_rsc" and not "${rscCfg.query}", so every navigation request would be a URL `
        + 'your site never serves.'
      : ' It happens to be the default, so nothing changes today — and nothing would say so if you changed it.';
    warn('rsc.query', `the generator reads rsc.param; nothing reads rsc.query. Rename the key.${lost}`);
  }
